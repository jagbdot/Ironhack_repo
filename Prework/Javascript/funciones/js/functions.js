...
function dniLetter( dni ) {
	...
	var result = lockup.charAt(remainder)
		...
		else if ( Number.isInteger(dni) == false ) {
			return 'Invalid parameter';
		}
...
// 		if (array_words[i].length > longest.length) {
...
array_lengths.push( findLongestWord( ["Temuco", "Santiago", "Talca", "valparaíso"] ) );
array_lengths.push( findLongestWord( ["Araucania", "Metropolitana", "Maule", "valparaíso_sanantonio"] ) );
...
function calculateAverage( array ) {
	...
	for (var i = 0; i < array.length; i++) {
		total_arrays += array[i];
	}
	var average = total_arrays / array.length;
	return average;
}
